...
# Import high-priority enhancement modules
from cache_manager import cache
from rate_limiter import rate_limiter, rate_limited
from telemetry import telemetry
from database import db_store
from bulk_operations import create_bulk_operations
...
# =============================================================================
# Bulk Operations Tools (High-Priority Enhancement #8)
# =============================================================================

@mcp.tool(
    annotations=ToolAnnotations(
        title="Send Bulk Messages", openWorldHint=True, destructiveHint=True
    )
)
async def send_bulk_messages(
    ...
    Args:
        chat_ids: List of chat IDs or usernames to send message to
        message: Message text to send
        delay_seconds: Delay between sends to avoid rate limits (default: 1.0)
        
    Returns:
        JSON summary of successful and failed operations
    """
    try:
        bulk_ops = create_bulk_operations(client, rate_limiter)
        return await bulk_ops.send_bulk_messages(chat_ids, message, delay_seconds)
    except Exception as e:
        return log_and_format_error("send_bulk_messages", e, ErrorCategory.MSG)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Delete Bulk Messages", openWorldHint=True, destructiveHint=True
    )
)
async def delete_bulk_messages(
    ...
    Args:
        ...
        delay_seconds: Delay between deletions (default: 0.5)
        
    Returns:
        JSON summary of deletion results
    """
    try:
        bulk_ops = create_bulk_operations(client, rate_limiter)
        return await bulk_ops.delete_bulk_messages(chat_id, message_ids, delay_seconds)
    except Exception as e:
        return log_and_format_error("delete_bulk_messages", e, ErrorCategory.MSG)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Mark Bulk Chats as Read", openWorldHint=True, destructiveHint=True
    )
)
async def mark_bulk_as_read(
    ...
    Args:
        chat_ids: List of chat IDs to mark as read
        delay_seconds: Delay between operations (default: 0.5)
    ...
    try:
        bulk_ops = create_bulk_operations(client, rate_limiter)
        return await bulk_ops.mark_bulk_as_read(chat_ids, delay_seconds)
    except Exception as e:
        return log_and_format_error("mark_bulk_as_read", e, ErrorCategory.CHAT)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Batch Get Chat Info", openWorldHint=True, readOnlyHint=True
    )
)
async def batch_get_chat_info(
    ...
    """
    Get information for multiple chats in batch.
    
    Args:
        chat_ids: List of chat IDs to get info for
        delay_seconds: Delay between requests (default: 0.5)
        
    Returns:
        JSON with chat information for all requested chats
    """
    try:
        bulk_ops = create_bulk_operations(client, rate_limiter)
        return await bulk_ops.batch_get_chat_info(chat_ids, delay_seconds)
    except Exception as e:
        return log_and_format_error("batch_get_chat_info", e, ErrorCategory.CHAT)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Export Bulk Contacts", openWorldHint=True, readOnlyHint=True
    )
)
async def export_bulk_contacts(format: str = "json") -> str:
    """
    ...
    Returns:
        Exported contacts in specified format
    """
    try:
        bulk_ops = create_bulk_operations(client, rate_limiter)
        return await bulk_ops.export_bulk_contacts(format)
    except Exception as e:
        return log_and_format_error("export_bulk_contacts", e, ErrorCategory.CONTACT)
...
@mcp.tool(
    annotations=ToolAnnotations(
        title="Get Cache Statistics", openWorldHint=True, readOnlyHint=True
    )
)
async def get_cache_stats() -> str:
    ...
    Returns:
        JSON with cache statistics including hit rate and entry count
    """
    try:
        stats = cache.get_stats()
        return json.dumps(stats, indent=2)
    except Exception as e:
        return log_and_format_error("get_cache_stats", e)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Get Rate Limiter Statistics", openWorldHint=True, readOnlyHint=True
    )
)
async def get_rate_limiter_stats() -> str:
    """
    ...
    try:
        ...
        return log_and_format_error("get_rate_limiter_stats", e)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Search Messages in Database", openWorldHint=True, readOnlyHint=True
    )
)
async def search_messages_db(
    ...
    """
    Search messages in local database with full-text search.
    ...
    """
    try:
        results = await db_store.search_messages(query, chat_id, limit)
        return json.dumps(results, indent=2, default=str)
    except Exception as e:
        return log_and_format_error("search_messages_db", e, ErrorCategory.MSG)


@mcp.tool(
    annotations=ToolAnnotations(
        title="Get Chat Statistics", openWorldHint=True, readOnlyHint=True
    )
)
async def get_chat_statistics(chat_id: Union[int, str]) -> str:
    """
    Get comprehensive statistics for a chat from database.
    
    Args:
        chat_id: Chat ID to get statistics for
        
    Returns:
        JSON with chat statistics including message count, top senders, etc.
    """
    try:
        ...
        stats = await db_store.get_chat_stats(chat_id)
        return json.dumps(stats, indent=2, default=str)
    except Exception as e:
        return log_and_format_error("get_chat_statistics", e, ErrorCategory.CHAT)


async def _main() -> None:
    try:
        # Initialize high-priority enhancements
        print("[Enhancements] Initializing database layer...")
        await db_store.initialize()
        
        print("[Enhancements] Starting cache cleanup task...")
